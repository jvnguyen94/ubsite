...
def train(model, device, trainloader, optimizer, epoch):
  ...
  for count, data in enumerate(trainloader):
    data = data.
    optimizer.zero_grad()
    output = model(prot_1)
    ...
    loss = loss_func(output, data.y.float().to(device))
    #for 
    loss.backward()
    optimizer.step()
  ...
  ## use bal_acc or AUROC  for the first pass
  print(
      f'Epoch {epoch-1} / 30 [==============================] - train_loss : {loss} - train_accuracy : {acc_tr}')
...
for epoch in range(num_epochs):
  ...
  if (accuracy > best_accuracy):
    ...
    torch.save(model.state_dict(), "../human_features/GCN.pth")
    print("Model")
  if (loss < min_loss):
    epochs_no_improve = 0
    min_loss = loss
